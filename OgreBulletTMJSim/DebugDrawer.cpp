...
{
	ColourValue c((float)color.getX(), (float)color.getY(), (float)color.getZ());
	c.saturate();
...
{
	ColourValue c((float)color.getX(), (float)color.getY(), (float)color.getZ(), (float)alpha);
	c.saturate();
...
void DebugDrawer::drawContactPoint(const btVector3 &PointOnB, const btVector3 &normalOnB, btScalar distance, int lifeTime, const btVector3 &color)
{
	...
	p.color.r = (float)color.x();
	p.color.g = (float)color.y();
	p.color.b = (float)color.z();
}
